...
class Ecu {
  ...
  start(ip, port) {
    ...
    this.adapter.log.info("Ecu.start() - done");
  }
   ...
   */
    ...
    if (this.timeoutRealTimeData != null) {
      clearInterval(this.timeoutRealTimeData);
      this.timeoutRealTimeData = null;
    }

    if (this.timeoutPowerOfDay != null) {
      clearInterval(this.timeoutPowerOfDay);
      this.timeoutPowerOfDay = null;
    }

    if (this.timeoutCyclicRequests != null) {
      clearInterval(this.timeoutPowerOfDay);
      this.timeoutCyclicRequests = null;
    }
    ...
    if (this.client != 0) {}
      this.client.end(() => {
        this.adapter.log.info("Ecu.end() - done");
        this.client = null;
    });
  }
  ...
  /*
   */
  enableRealTimeData(seconds) {
    this.timeoutRealTimeData = setInterval(() => {

      this.adapter.addEventListener('rsp', () => {
        ;
      });
      this.reqRealTimeData();
    }, seconds * 1000);
  ...
  /*
  */
  cyclicRequests(seconds) {

    // interval min 10sec, default 75sec
    let miliseconds = 75 * 1000;
    if ( typeof(seconds) === 'number' && seconds != null ) {
      miliseconds = (seconds < 10) ? 10000 : seconds * 1000;
    } 

    // this.adapter.log.info("Ecu.cyclicRequests() miliseconds: " + miliseconds);

    this.timeoutCyclicRequests = setInterval( () => {

      if ( this.waitForResponse === false ) {
        switch (this.reqService) {
  ...
  /*
   */
  enablePowerOfDay(seconds) {
    if (this.timeoutPowerOfDay != null) {
      clearInterval(this.timeoutPowerOfDay);
    }
    this.timeoutPowerOfDay = setInterval(() => {
      //reqPowerOfDay('20211018');
      this.reqEnergyOfWMY("year");
    }, seconds * 1000);
  }
  ...
  reqSystemInfo() {
    if (!this.waitForResponse) { 
      ...
      this.adapter.log.info("ecu.reqSystemInfo: " + req);
      return true;
    }
    return false;
  }
  ...
  reqRealTimeData() {
    if (!this.waitForResponse) { 
      if (this.ecuId != null) {
        ...
        this.waitForResponse = true;
        this.client.write(req);
        this.adapter.log.info("reqRealTimeData: " + req);
      } else {
    ...
    if (!this.waitForResponse) { 
      if (this.ecuId != null) {    
        ...
        if (this.ecuId != null) {
          ...
          this.waitForResponse = true;          
          this.client.write(req);
          this.adapter.log.info("reqPowerOfDay: " + req);
        } else {
  ...
  reqInverterSignalLevel() {
    if (!this.waitForResponse) { 
      if (this.ecuId != null) {    
        ...
        this.waitForResponse = true;             
        this.client.write(req);
        this.adapter.log.info("reqInverterSignalLevel: " + req);
      } else {
  ...
  */
    ...
    if (wmyStr != "nok") {
      if (!this.waitForResponse) { 
        if (this.ecuId != null) {    
          ...
          this.waitForResponse = true;     
          this.client.write(req);
          this.adapter.log.info("reqEnergyOfWMY: " + req);
        } else {
          this.adapter.log.info("reqEnergyOfWMY() ecuId: " + this.ecuId);
        }
      }
    }
  }
  ...
  decodeRsp = (ecuRsp) => {
    this.waitForResponse = false;  
    this.adapter.log.info("ecu.rsp:" + bin2HexAscii(ecuRsp));
  ...
  decodeSystemInfo(rsp) {
    ...
    this.adapter.log.info("rspSystemInfo - ok");
  }
    ...
    if (inv.state == "01") {
      switch (inv.inverterType) {
        case "01": { // YC600 todo  not jet supported
            ...
            this.adapter.log.info("rspRealTimeData.inverterType: " + inv.inverterType + " not supported");
          }
          break;
        ...
        case "03": { // QS1
            ...
            this.inverters[inverterId] = qs1Prefix;
            this.adapter.setState(qs1Prefix + ".online", true, true); // todo
        ...
        default:
          this.adapter.log.info("rspRealTimeData.inverterType: " + inv.inverterType + " unknown");
          break;
      }
    }
    this.adapter.log.info("rspRealTimeData - ok");
  }
  ...
  decodePowerOfDay(rsp) {
    ...
    this.adapter.log.info("rspPowerOfDay - ok");
  }
  ...
  decodeEnergyOfWMY(rsp) {
    ...
    this.adapter.log.info("rspEnergyOfWMY (" + egy.energyWMY + ") - ok");
  }
  ...
  decodeInverterSignalLevel(rsp) {
    ...
    if (isl.status == "00") {
      ...
      for (let len = rsp.subarray(idx).length - 2 - 7; len > 0; len -= 7) {
        ...
        let prefix = this.inverters[isl.inverterId];
        if (prefix) {
    ...
    this.adapter.log.info("rspInverterSignalLevel - ok");
  }
