...
class Ecu {
  ...
  init() {
    ...
    this.adapter.getForeignObject('system.config', (err, data) => {
      ...
      this.jobMidnight = schedule.scheduleJob( '1 0 * * *', (fireDate) => {
        this.scheduleSunsetSunrise();
      })
  ...
  */
  ...
  decodeRsp(ecuRsp) {
    ...
    let idx = 0;
    let commandNumber = this.decodeHdr(ecuRsp);
    idx += 13;  // skip header
  ...
  decodeHdr(rsp) {
    ...
    try {
      if (rspLen > 17) {  // hdr len + end signature len
        if (rsp.subarray(rspLen-4).toString() === (REQ_END + '\n') /*rsp.subarray(rsp.len-4).toString === REQ_END + `\r`*/) {
  ...
  decodeAndProcessRealTimeData(rsp) {
    ...
    inv.dateTime = bcd2JS_ISO_Date(rsp.subarray(idx, (idx += 7)));

    if (rtd.matchStatus = '00') { // semantic not clear

      // inverter loop .. rtd.inverter > 1 TODO not tested!!! 
  ...
  /*
    ...
    * @param { } [inv]
  */
  async processRealTimeData(inverterId, inv) {
    ...
    switch (inv.inverterType) {
      ...
      case '03': { // QS1
          ...
          this.createAndSetInverterQs1Objects(inv, qs1Prefix); /*
   ...
   */
  async createAndSetInverterQs1Objects(inv, prefix) {
    let createObjects = true; // todo
    this.adapter.log.debug(`ecu.createAndSetInverterQs1Objects(${prefix}) ...` );
    await this.adapter.getState(prefix + '.online', (err, obj) => {
      createObjects = (obj) ? true : true;  // todo
    });
      
    await this.createInverterQs1Objects(createObjects, prefix);
    await this.setInverterQs1Objects(inv, prefix);

    this.adapter.log.debug(`ecu.createAndSetInverterQs1Objects(${prefix}) - done` );
  }

  async setInverterQs1Objects(inv, prefix) {
    this.adapter.log.debug(`ecu.setInverterQs1Objects(${prefix}) ...` );    
    await this.adapter.setState(prefix + '.online', (inv.state == '01'), true); // todo
    await this.adapter.setState(prefix + '.inverter_id', inv.inverterId, true);
    await this.adapter.setState(prefix + '.date_time', inv.dateTime, true);
    await this.adapter.setState(prefix + '.frequency', inv.frequency, true);
    await this.adapter.setState(prefix + '.ac_voltage',parseInt(inv.voltage),true);
    await this.adapter.setState(prefix + '.temperature',inv.temperature,true);
    await this.adapter.setState(prefix + '.dc_power1',parseInt(inv.power1),true);
    await this.adapter.setState(prefix + '.dc_power2',parseInt(inv.power2),true);
    await this.adapter.setState(prefix + '.dc_power3',parseInt(inv.power3),true);
    await this.adapter.setState(prefix + '.dc_power4',parseInt(inv.power4),true);
    await this.adapter.setState(prefix + '.dc_power',inv.power1 + inv.power2 + inv.power3 + inv.power4,true);
    this.adapter.log.debug(`ecu.setInverterQs1Objects(${prefix}) - done` );        
  }

  async createInverterQs1Objects(createObjects, prefix) {
    this.adapter.log.debug(`ecu.createInverterQs1Objects(${createObjects}, ${prefix}) ...` );    
