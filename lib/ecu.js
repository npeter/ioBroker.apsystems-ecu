...
class Ecu {
   ...
   */
  async init() {
    this.hideEcuId = this.adapter.config.hide_ecu_id;
  ...
  sm(event) {
    ...
    this.adapter.log.debug(`sm(${event}) state=${this.state}`)
    ...
    switch (this.state) {
      ...
      case 'stWaitForConnect':
        if (event === 'evConnected') {
          ...
          this.trNextService('evRepeatService');
        }
        break;

      case 'stWaitForResponse':
        if (event === 'evResponseOk') {
          this.trNextService('evNextService');  // evNoNextService
        } 
        else if (event === 'evResponseTimeout') {
          this.trNextService('evRepeatService');  // evNoNextService
        }
        else if (event === 'evNoNextService') {
          this.state = 'stWaitForDisconnected';
          this.trDisconnect();  // evDisconnected
        }
        break;
      ...
      case 'stWaitForNextCycle':
        if (event === 'evNextCycle') {
          this.state = 'stWaitForConnect';
          this.trConnect();
        }
        else if (event === 'evCmdStartStop') {   //TODOTODO 
          this.state = 'stWaitForConnect';
          clearTimeout(this.cyclicRequestsTimeoutId);          
          this.trConnect();
        }
        break;
  ...
  async trConnect() {
    ...
    this.client.on('error', (error) => {
      this.sm('evSocketError'); 
    });

    this.client.on('timeout', () => {
      this.sm('evSocketTimeout');
    });

    this.client.on('data', (ecuRsp) => {
      let serviceTime = Date.now() - this.reqStartTime;
      this.sm(this.decodeRsp(ecuRsp)); 
     });
  ...
  trDisconnect() {
    ...
    this.adapter.log.debug('Ecu.trDisconnect()');   
    this.sm('evDisconnected');
  }
  ...
  async trNextService(event) {
    ...
    if (this.serviceState === 'stSystemInfo' || this.ecuId == null) {
      if (event == 'evRepeatService' || this.ecuId == null) {
        ...
        await this.adapter.log.debug(`trNextService(${event}) ${oldServiceState}->${this.serviceState}`);
      }
      else {
  ...
  trNextCycle() {
    ...
    this.cyclicRequestsTimeoutId = setTimeout( () => {
      // if sun
      // 
      this.sm('evNextCycle');
    }, _intervalMsec);
    this.adapter.log.debug(`trNextCycle() ${_intervalMsec/1000}s`)
  }



  reqService(serviceInfo, req) {
