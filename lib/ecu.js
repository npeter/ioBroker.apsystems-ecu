...
class Ecu {
  ...
  async ecuStateMachine(event) {
    ...
    if (event === 'evSocketError' || event === 'evSocketTimeout' || event === 'evInvalidResponse') {
      this.adapter.log.debug(`Ecu.ecuStateMachine(${event}) ${oldState} -> ${this.state}`)
  ...
  async trDisconnect() {
    ...
    clearInterval(this.nextCycleTimeoutId);
    this.nextCycleTimeoutId = null;      
   ...
   * - recursive call with 'evRepeatService' to find next optional service
   * @param {*} event 
   */
  ...
  async trNextCycle() {
    ...
    await this.adapter.log.debug(`Ecu.trNextCycle() ${_intervalMsec/1000}s delay`)
  }
  ...
  */
  ...
  decodeRsp(ecuRsp) {
    ...
    this.adapter.log.debug('Ecu.decodeRsp() - ' + bin2HexAscii(this.hideEcuIdInRsp(ecuRsp).subarray(0,ecuRsp.length-1)));
    this.rspWatchDogDisable();     
