...
class Ecu {
  ...
  constructor(adapter) {
    ...
    this.astroTimeFunc = function () {
      return SunCalc.getTimes(new Date(), 49.8648048, 9.601144);
    }

    this.jobStartFunc = function () {
      return schedule.scheduleJob(this.astroTime.sunRise, (fireDate) => {
        this.adapter.log.inf(`jobStart at ${fireDate}`);
      });  
    }

    this.jobEndFunc = function () {
      let job = schedule.scheduleJob(this.astroTime.sunSet, (fireDate) => {
        this.adapter.log.inf(`jobEnd at ${fireDate}`);
      });  
      return job;
    }

    this.astroTime = this.astroTimeFunc();
    this.jobStart = this.jobStartFunc();
    this.jobEnd = this.jobEndFunc();
    ...
    this.jobMidnight = schedule.scheduleJob( '1 0 * * *', (fireDate) => {
      ...
      this.jobStart = this.jobStartFunc();
      this.jobEnd = this.jobEndFunc();
      this.adapter.log.inf(`jobMidnight at ${fireDate}`);      
    })
