...
class Ecu {
  ...
  async onStateChange(id, state) {
    ...
    // trigger power_of_day service for new 'day'
    ...
    else { // set trigger for requested service 
      if (state.val == true) {
  ...
  decodeHdr(rsp) {
    ...
    try {
      if (rspLen > 17) {  // hdr len + end signature len
        let a = rsp.subarray(rspLen-4).toString();
        let b = REQ_END + `\n`
        if (a===b/*rsp.subarray(rsp.len-4).toString === REQ_END + `\r`*/) {
      ...
      return (hdr.commandNumber) ? hdr.commandNumber : 'error';   
    }   
}
