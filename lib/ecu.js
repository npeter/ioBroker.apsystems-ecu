...
class Ecu {
  ...
  /**
   * Jobs tob be done at midnight
   * - update total_energy_yesterday from current_day_energy
   *  - current_day energy has tob reset by ECU
   * - update dc_peak_power_yesterday by dc_peak_power_today and reset dc_peak_power_today
   * - reset service_count
  */
  async doMidnightJobs() {
    let tmp_current_day_energy = await this.adapter.getStatesAsync(this.CURRENT_DAY_ENERGY);  
    this.adapter.setState(this.TOTAL_ENERGY_YESTERDAY, tmp_current_day_energy, true);
    ...
    this.adapter.setState(this.SERVICE_COUNT_ID, 0);
  ...
  */
  ...
  async createInverterYc60Objects(createObjects, inverterTypeStr, prefix) {
    ...
    await this.adapter.log.debug(`Ecu.createInverterYc60Objects(${createObjects}, ${prefix}) done` ); 
  }
