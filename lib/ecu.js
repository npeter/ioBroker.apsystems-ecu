...
const INV_ID_TYPE_YC600 = 40;
const INV_ID_TYPE_YC100 = 50;
const INV_ID_TYPE_DS3 = 70;
const INV_ID_TYPE_QS1 = 80;
  ...
  async trServiceSM(event) {
    ...
    if (this.serviceState === 'stSystemInfo' || this.ecuId == null) {
      if (event == 'evRepeatService' || this.ecuId == null) {
        await this.adapter.log.debug(`Ecu.trServiceSM(${event}/${this.ecuId}) ${oldServiceState}->${this.serviceState}`);
        this.reqService('SYSTEMINFO', REQ_SYSTEMINFO + REQ_END + '\n');
      }
      else {
        this.serviceState = 'stRealTimeData';
        await this.adapter.log.debug(`Ecu.trServiceSM(${event}) ${oldServiceState}->${this.serviceState}`);
      ...
      if (event == 'evRepeatService') {
        ...
        if (this.cmdPowerOfDay) {
          ...
          await this.adapter.log.debug(`Ecu.trServiceSM(${event}) ${oldServiceState}->${this.serviceState} - recursion`);
          this.trServiceSM('evRepeatService');
        }
      }
    }
      ...
      if (this.cmdEnergyOfWeek && event == 'evRepeatService') {
        await this.adapter.log.debug(`Ecu.trServiceSM(${event}) ${oldServiceState}->${this.serviceState}`);
        this.reqService('ENERGY_OF_WMY', REQ_ENERGY_OF_WMY + this.ecuId + REQ_END + REQ_WEEK + REQ_END + '\n');  // week
      }
      else {
      ...
      if (this.cmdEnergyOfMonth && event == 'evRepeatService') {
        await this.adapter.log.debug(`Ecu.trServiceSM(${event}) ${oldServiceState}->${this.serviceState}`);
        this.reqService('ENERGY_OF_WMY', REQ_ENERGY_OF_WMY + this.ecuId + REQ_END + REQ_MONTH + REQ_END + '\n');
      }
      else {
      ...
      if (this.cmdEnergyOfYear && event == 'evRepeatService') {
        await this.adapter.log.debug(`Ecu.trServiceSM(${event}) ${oldServiceState}->${this.serviceState}`);
        this.reqService('ENERGY_OF_WMY', REQ_ENERGY_OF_WMY + this.ecuId + REQ_END + REQ_YEAR + REQ_END + '\n');
      }
      else {
      ...
      await this.adapter.log.error(`Ecu.trServiceSM(${event}) +++ invalid state: ${oldServiceState}`);
    }
  }
*/
  ...
  */
  ...
  decodeRsp(ecuRsp) {
    ...
    switch (commandNumber) {
      ...
      case '0030': // inverterSignalLevel
        ...
        break;
      default:
        this.adapter.log.error('Ecu.decodeRsp() +++ unknown commandNumber: ' + commandNumber);
  ...
  async createAndSetInverterObjects(inv, inverterTypeStr) {
    ...
    await this.createInverterYc60AndDs3Objects(createObjects, inverterTypeStr, prefix);
   ...
   * @param {string} prefix - device object id
   */
  async createInverterYc60AndDs3Objects(createObjects, inverterTypeStr, prefix) {
    if (!createObjects || inverterTypeStr != INV_TYPE_STR_YC600) {
      this.adapter.log.silly(`Ecu.createInverterYc60AndDs3Objects(${createObjects}, ${prefix}) done`);
      return;
    }
    this.adapter.log.debug(`Ecu.createInverterYc60AndDs3Objects(${prefix}) creating ...`);
    ...
    await this.adapter.log.debug(`Ecu.createInverterYc60AndDs3Objects(${createObjects}, ${prefix}) done`);
  }
